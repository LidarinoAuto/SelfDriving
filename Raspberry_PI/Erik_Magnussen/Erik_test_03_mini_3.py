...
ROTATION_SPEED = 2                # (brukes til ESP32-kommandoer)
ROTATION_TIME_90_DEG = 1.5        # Tid for en 90�-rotasjon

current_distance_lidar = 9999
running = True

# Global pose representert som en 3x3 homogen transformasjonsmatrise (SE2)
...
def twist_to_transform(vx, vy, omega, dt):
    ...
    vx og vy er i mm/s, omega i rad/s.
    """
    if abs(omega) < 1e-6:
        dx = vx * dt
        dy = vy * dt
...
def update_pose(vx, vy, omega, dt):
    ...
    current_pose = np.dot(current_pose, increment)
...
def check_ultrasound():
    """Returnerer True dersom minst �n ultralydsensor registrerer en hindring innenfor STOP_DISTANCE_ULTRASOUND."""
    data = read_all_ultrasound()
    for navn, avstand in data.items():
    ...
    try:
        ...
        while True:
            ...
            if check_ultrasound():
                stop_robot()
                time.sleep(2)
                choose_di     .gimport RPi.GPIO as GPIO
import serial
import time
import threading
import math
