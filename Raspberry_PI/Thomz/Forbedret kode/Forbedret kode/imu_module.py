...
# --- Bias Calibration ---
        skriv_logg("Calibrating gyro bias. Keep robot completely still...") # Updated skriv_logg
...
def rotate_by_gyro(target_angle_degrees):
    ...
    """
    global integrated_angle, last_time, imu_initialized, bias_calibration_done
    ...
    while abs(target_angle_degrees - integrated_angle) > angle_tolerance and (time.time() - start_rotation_time) < rotation_timeout:
        ...
        time.sleep(0.01) # Small delay to control loop speed 
    ...
    if abs(final_error) <= angle_tolerance:
        ...
        pass # We log completion in main.py or rotate_to_heading
...
def rotate_to_heading(target_heading_degrees):
    """
    ...
    # --- Rotation Control Parameters (Adjust these for tuning) ---
    # Simple proportional control: Speed = Kp_heading * Error
    Kp_heading = 0.6 # Proportional gain for heading control (Adjust this!)
    heading_tolerance = 3.0 # Stop when within +/- 3.0 degrees of target heading (Adjust!)
    base_speed_heading = 25 # Max/Base motor speed for heading rotation (Adjust if needed)
    ...
    while (time.time() - start_rotation_time) < rotation_timeout:
        current_heading = compass_module.read_compass() # <-- LES KOMPASS HEADING

        if current_heading == -1.0:
            skriv_logg("Warning: Compass reading failed during heading rotation. Stopping.")
            motor_control.send_command("0 0 0\n")
            break # Exit loop on compass error

        # Calculate heading error (difference between target and current heading)
        # Handle wrap-around (e.g., turning from 10 to 350 is -20, not +340)
        error = target_heading_degrees - current_heading

        # Adjust error for wrap-around (shortest path)
        if error > 180:
            error -= 360
        elif error < -180:
            error += 360
        ...
        if abs(error) <= heading_tolerance:
            skriv_logg(f"Heading rotation completed. Reached {current_heading:.2f}  within {target_heading_degrees:.2f}  target.")
    ...
    # Ensure motors stop when loop finishes (either by target or timeout)
    motor_control.send_command("0 0 0\n")
    time.sleep(0.5) # Liten pause

    # Log final state or reason for loop exit AFTER the loop
    end_time = time.time()
    duration = end_time - start_rotation_time
    current_heading_final = compass_module.read_compass() # Read final heading

    if abs(target_heading_degrees - current_heading_final) <= heading_tolerance:
        ...
        skriv_logg(f"Heading rotation to {target_heading_degrees:.2f}  timed out after {duration:.2f}s. Final heading: {current_heading_final:.2f} , Remaining error: {target_heading_degrees - current_heading_final:.2f} .")            