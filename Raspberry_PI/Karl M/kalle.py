...
ROTATION_TIME_90_DEG = 1.5       # Sekunder for 90� rotasjon

# Ultralydsensor-pinner (sensorrekkef�lge: front venstre, front h�yre, bak venstre, bak h�yre)
...
class UltrasoundSensor:
    ...
    def get_distances(self):
        distances = []
        for trig, echo, name in zip(self.trig_pins, self.echo_pins, self.sensor_names):
            d = self.read_distance(trig, echo)
            distances.append(d)
            if d > 0:
                logging.info(f"{name}: {d:.2f} cm")
            else:
                logging.info(f"{name}: ingen m�ling")
        return distances
    ...
    def check_obstacle(self, stop_distance):
        distances = self.get_distances()
        for i, d in enumerate(distances):
            if d > 0 and d < stop_distance:
                logging.info(f"Hindring oppdaget av sensor {i} p� {d:.2f} cm")
                return True
        return False
...
class IMUHandler:
    ...
    def get_gyro_z(self):
        # Henter gyroskopverdien for z-aksen (yaw)
...
class RobotController:
    ...
    def rotate_by_angle(self, angle):
        """
        Roterer roboten med en gitt vinkel (grader).  
        Negative vinkler = venstresving, positive = h�yresving.
        Bruker IMU for � integrere den faktiske rotasjonshastigheten.
        """
        ...
        while time.time() - start_time < rotation_time:
            ...
            angular_rate = self.imu.get_gyro_z()  # grader per sekund
    ...
    def choose_direction_and_rotate(self):
        ...
        # Kartlegging av sensorindeks til rotasjonsvinkel:
        # front_left -> -30�, front_right -> 30�, back_left -> -150�, back_right -> 150�
        if best_index == 0:
            angle = -30
        elif best_index == 1:
            angle = 30
            ...
            angle = 0
        logging.info(f"Valgt retning: sensor {best_index} med {best_distance:.2f} cm, roterer {angle}�")
    ...
    def avoid_obstacle(self):
        """
        Felles metode for � h�ndtere hindringer: stopp, roter mot fri retning,
        kj�r en kort distanse fremover, og fortsett deretter.
        """
        self.stop_robot()
        time.sleep(1)
    ...
    def run(self):
        ...
        try:
            while True:
                # Prioriterer ultralyd: sjekk for hindringer
                if self.ultrasound.check_obstacle(STOP_DISTANCE_ULTRASOUND):
                    self.avoid_obstacle()
                    continue
                # Sjekk LiDAR dersom ingen hindring med ultralyd
