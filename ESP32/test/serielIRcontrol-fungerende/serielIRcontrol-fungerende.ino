...
// (både IR og Serial vil kunne sette disse)
...
void IR_Check();  // Ny funksjon for IR
...
// -----------------------------------------------------
void loop() {
  // 1) Les kommandoer fra Serial
  readSerialCommands();

  // 2) Sjekk IR-kommandoer
  IR_Check();

  // 3) Oppdater KiwiDrive-setpunkter (sum av evt. IR/Serial)
  //    Begge funksjoner skriver direkte til vx_mm, vy_mm, omega_rad
  KiwiDrive(vx_mm, vy_mm, omega_rad);

  // 4) Kjør PID hver ~100ms
...
void readSerialCommands() {
  if (Serial.available() > 0) {
    ...
    line.trim();
    if (line.length() > 0) {
      // Forsøk å parse inn i 3 floats: vx, vy, omega
      float x, y, w;
...
* - Nullstiller til 0,0,0 hvis ingen IR-kommandoer
*   er mottatt innen IR_TIMEOUT.
*******************************************************/
void IR_Check() {
  // Dersom vi har mottatt noe fra IR
  ...
  // Sjekk om det er lenge siden siste IR-mottak
  if (millis() - lastIRReceiveTime > IR_TIMEOUT) {
    // Nullstill hvis ingen ny kommando
    // (NB: Kan endres hvis du ønsker å kombinere IR + Serial)
    vx_mm = 0.0;
    vy_mm = 0.0;
    omega_rad = 0.0;
  }
...
/*******************************************************
* updatePID()
* - Ligner på din eksisterende kode
*******************************************************/
void updatePID(int motorID,
               volatile long &encoderCount,
               long &lastCount,
